...
def load_tickers():
    try:
        ...
        print(f"שגיאה בקריאת tickers.csv: {e}")
        return ["TSLA", "AAPL", "MSFT"]  # ברירת מחדל אם הקובץ לא נמצא
...
def scan_stocks(limit=4):
    ...
    for t in tickers:
        try:
            ...
            unusual_vol = today['Volume'] > (1.5 * avg_vol)

            if abs(change) >= 2 and unusual_vol:
                entry = today['Close']
                stop = entry * 0.97
